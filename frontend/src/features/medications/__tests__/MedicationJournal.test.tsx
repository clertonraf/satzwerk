...
import type { MedicationJournalEntry, MedicationJournalView } from '../types'
...
function makeJournal(days: MedicationJournalView['days'] = []): MedicationJournalView {
  return { days }
}
...
describe('MedicationJournal', () => {
  ...
  it('renders journal entries grouped by date', async () => {
    mockGetJournal.mockResolvedValue(
      makeJournal([
        {
          date: '2026-08-02',
          entries: [makeEntry({ taken: true, takenAt: '2026-08-02T08:00:00Z', medicationName: 'Vitamin D' })],
        },
        {
          date: '2026-08-01',
          entries: [makeEntry({ id: 'log-2', taken: false, takenAt: '2026-08-01T09:00:00Z', medicationName: 'Magnesium' })],
        },
      ]),
    )
    renderJournal()
  ...
  it('shows medication name and dosage info', async () => {
    mockGetJournal.mockResolvedValue(
      makeJournal([{ date: '2026-08-02', entries: [makeEntry({ dosageAmount: 500, dosageUnit: 'MG' })] }]),
    )
    renderJournal()
    expect(await screen.findByText('Vitamin D')).toBeInTheDocument()
  ...
  it('shows notes when present', async () => {
    mockGetJournal.mockResolvedValue(
      makeJournal([{ date: '2026-08-02', entries: [makeEntry({ notes: 'With food' })] }]),
    )
    renderJournal()

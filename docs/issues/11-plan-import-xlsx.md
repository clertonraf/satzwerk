...
Allow a user to upload an `.xlsx` spreadsheet (exported from a personal trainer's tool) to create a WorkoutPlan. Parsing is delegated to the satzwerk-parser sidecar service.

## Decisions (from ADR-0004 and grilling session)

| Question | Decision |
|---|---|
| Architecture | Satzwerk backend calls satzwerk-parser internally; frontend uploads to one endpoint |
...
3. **satzwerk-parser client**: WebClient bean calling `POST http://satzwerk-parser:8080/api/workout/parse`
...
6. **Error handling**: satzwerk-parser unavailable → 503; parse error → 422
...
1. Add `satzwerk-parser` service to `docker-compose.yml` using `ghcr.io/clertonraf/satzwerk-parser:latest`
2. Expose on internal network only; Satzwerk backend resolves `http://satzwerk-parser:8080`
...
- satzwerk-parser unavailable → 503
